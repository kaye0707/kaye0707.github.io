...
function tonav() {
  ...
  //修复没有弄右键菜单 的童鞋 无法回顶部的问题
  document.getElementById("page-name").innerText = document.title.split(" | Kaye")[0];
}
...
$.ajax({
  ...
  data: {
    key: '7MCBZ-WT4RQ-HD555-43YEY-BKMQT-4VBBW',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(110.925456, 21.662999, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
        ...
        case "广东省":
          switch (ipLoacation.result.ad_info.city) {
            case "广州市":
              ...
              break;
            case "深圳市":
              posdesc = "世界之窗打卡宝点，公明腊肠人间美味！";
              break;
            case "珠海市":
              posdesc = "情侣路浪漫无比，白藤湖蚝油更是家常必备。";
              break;
            case "佛山市":
              posdesc = "祖庙庄严肃穆，九江煎堆甜而不腻。";
              break;
            case "东莞市":
              posdesc = "虎门大桥气势恢宏，东莞腊味香气四溢。";
              break;
            case "中山市":
              posdesc = "孙中山故居肃然起敬，杏仁饼酥脆可口。";
              break;
            case "江门市":
              posdesc = "开平碉楼独具风情，新会陈皮更是久负盛名。";
              break;
            case "肇庆市":
              posdesc = "七星岩风景如画，裹蒸粽更是节庆必备。";
              break;
            case "惠州市":
              posdesc = "西湖一片宁静，惠州梅菜家家爱吃。";
              break;
            case "梅州市":
              posdesc = "围龙屋极具客家特色，梅州柚清甜多汁。";
              break;
            case "汕头市":
              posdesc = "小公园热闹非凡，牛肉丸Q弹爽口~。";
              break;
            case "汕尾市":
              posdesc = "红海湾碧海蓝天，咸鱼味道更独特。";
              break;
            case "潮州市":
              posdesc = "开元寺古朴典雅，凤凰单枞茶香四溢。";
              break;
            case "揭阳市":
              posdesc = "黄满寨瀑布壮观，普宁豆干口感丰富。";
              break;
            case "云浮市":
              posdesc = "石材雕刻工艺精湛，罗定豆豉味道浓郁。";
              break;
            case "韶关市":
              posdesc = "丹霞山奇峰林立，南雄板栗香甜糯口~。";
              break;
            case "茂名市":
              posdesc = "放鸡岛风光旖旎，高州荔枝个大汁多。";
              break;
            case "湛江市":
              posdesc = "湖光岩火山口湖神秘莫测，对虾新鲜美味。";
              break;
            default:
              posdesc = "白切鸡呱呱好味哇~";
              break;
  ...
  else if (date.getHours() >= 16 && date.getHours() < 19) timeChange = "<span>夕阳无限好！看最美晚霞呀~！</span>";
  else if (date.getHours() >= 19 && date.getHours() < 24) timeChange = "<span>晚上好</span>，夜生活嗨起来！";
  else timeChange = "夜深了，早点休息，少熬夜哟。";
  ...
  debounce(function () {
    new Vue({
      data: function () {
        this.$notify({
          title: "嘿呀！复制成功啦🍬",
          message: "若要转载最好保留原文链接哦，给你一个大大的赞哦！",
          position: 'top-left',
          offset: 50,
          showClose: true,
          type: "success",
...
function createtime1() {
  var grt = new Date("2023/12/28 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Kaye %c 你正在访问Kayeの小窝",
  ...
  setTimeout(
    console.warn.bind(console, "%c S013-782 %c 你现在正处于监控中，请注意言行举止！", "color:white; background-color:#d9534f", "")
  );
}
createtime2();
...
function share_() {
  ...
  try {
    ...
    // var subTitle = title.endsWith("| Kayeの小窝🍉") ? title.substring(0, title.length - 14) : title;
    // 获取 `<h1>` 中的文章标题
    let articleTitle = document.querySelector('h1').innerText;
    navigator.clipboard.writeText('Kayeの小窝🍉的站内分享\n文章标题：' + articleTitle + '\n文章链接：' + url + '\n欢迎到访！🍖🍗🥩');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息啦~🎉",
...
document.addEventListener('visibilitychange', function () {
  if (document.hidden) {
    //离开当前页面时标签显示内容
    document.title = ' Σ(っ °Д °;)っ网站崩溃了！！！';
    clearTimeout(titleTime);
  } else {
    //返回当前页面时标签显示内容
    document.title = '抓到你啦🐖，ヾ(≧▽≦*)o';
    //两秒后变回正常标题
...
var calendarFormatter = {
  ...
  /**
  * 传入农历年月日以及传入的月份 是否 闰月获得详细的公历、农历object 信息 <=>JSON
  * @param y  lunar year
  * @param m  lunar month
  * @param d  lunar day
...
if (m == 8 && dd == 15) {//搞来玩的，小日子投降
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("萝卜头已经投降" + (y - 1945).toString() + "年了😃");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
if (m == 1 && dd == 1) {//元旦节
...
l = ["非常抱歉，因为不可控原因，小破站将于明天停止运营！", "好消息，日本没了！", "美国垮了，原因竟然是川普！", "微软垮了！", "你的电脑已经过载，建议立即关机！", "你知道吗？站长很喜欢你。", "一分钟有61秒哦", "今天男友/女友跟别人跑了！"]
...
function createtime() {
  ...
  var start = new Date("2022/08/01 00:00:00"); // 旅行者1号开始计算的时间
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://img.shields.io/badge/%E5%B7%A5%E4%BD%9C%E6%91%B8%E9%B1%BC%E4%B8%AD%F0%9F%90%9F-6adea8?style=social&logo=coffeescript&label=K%E5%B0%8F%E5%B1%8B' title='什么时候能够不用上班呀~'><br> <div style="font-size:13px;font-weight:bold">本站竟然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米🛰️，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://img.shields.io/badge/%E4%B8%8B%E7%8F%AD%E4%BC%91%E6%81%AF%E5%95%A6%F0%9F%98%B4-6adea8?style=social&logo=buymeacoffee&label=K%E5%B0%8F%E5%B1%8B' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站竟然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米🛰️，约为 ${unit} 个天文单位 🚀</div>`),
  ...
  setTimeout(function () {
    new Vue({
      data: function () {
        this.$notify({
          title: "温馨提醒🍒",
          message: " (｡･∀･)ﾉﾞ由于网站部分设置项更新，当前已为您重置所有设置，祝您阅读愉快！",
          position: 'top-left',
          offset: 50,
          showClose: true,
          type: "success",
...
if (localStorage.getItem("font") == undefined) {
  localStorage.setItem("font", "MaruSC");
}
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://tuchuang.voooe.cn/images/2024/07/29/default_page_cover.webp); 
    --darkmode-bg:url(https://tuchuang.voooe.cn/images/2024/07/29/dm8.webp);
    --mobileday-bg: url(https://tuchuang.voooe.cn/images/2024/07/29/mb4.webp); 
    --mobilenight-bg: url(https://tuchuang.voooe.cn/images/2024/07/29/default_cover_206.webp);
  }`;
}
// 切换背景主函数
function changeBg(s) {
...
function createWinbox() {
  ...
  // 每一类我放了一个演示，直接往下复制粘贴 a标签 就可以，需要注意的是 函数里面的链接 冒号前面 需要添加反斜杠\进行转义
...
<div class="content" style="display:flex">
  ...
  <div class="content-text" style="font-weight:bold; padding-left:20px"> 侧边显隐 (默认显示) </div><input type="checkbox" id="rightSideSet" onclick="toggleRightside()">
</div>


<h2>字体设置</h2>
<h4>非商免字体未经授权只能个人使用。本站为完全非商业、非盈利性质的网站，平时用于个人学习交流，如有侵权请联系站长删除，谢谢！ —— 致版权方</h4>
<p id="swfs">
<a class="swf" id="swf_ZZAW" href="javascript:;" rel="noopener external nofollow" style="font-family:'ZZAW'!important;color:black" onclick="setFont('ZZAW')">筑紫A丸标准体2.0</a>
<a class="swf" id="swf_MaruSC" href="javascript:;" rel="noopener external nofollow" style="font-family:'YR'!important;color:black" onclick="setFont('MaruSC')">MaruSC</a>
...
<h3>4. 自定义背景</h3>
<p><center>
<input type="text" id="pic-link" size="70%" maxlength="1000" placeholder="请输入有效的图片链接，如 https://xxx.com/img/home_bg.webp">
</center></p>
<p><center>
...
<br>
<center><div style="font-size:1.2em;color:var(--theme-color);font-weight:bold;">------💯❗我是一条分割线❗💯------</div></center>
<br>
...
function winResize() {
  try {
    ...
    console.log("Pjax毒瘤 抽风运行 winResize 方法🙄🙄🙄");
  }
}
